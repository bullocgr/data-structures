...
int size_of_tree(struct bst_node *bst_node);
int height_of_tree(struct bst_node* bst_node);
int has_path(int sum, struct bst_node *bst_node);
void slide_left(struct stack *stack, struct bst_node *node);
...
int bst_size(struct bst* bst) {
  int size = size_of_tree(bst->root);
  return size;
}

/*
this will get us the size of the tree by taking in a different argument
*/
...
int size_of_tree(struct bst_node* bst_node) {
  if(bst_node == 0) {
    ...
    return (size_of_tree(bst_node->left) + 1 + size_of_tree(bst_node->right)); //everytime you add a new node you add one if you hit null u stop
  }
}
...
int bst_height(struct bst* bst) {
  return height_of_tree(bst->root);
}

/*
This will get us the height while taking in the node instead of the bst
*/

int height_of_tree(struct bst_node* bst_node) {
  int right_depth, left_depth;
  if(bst_node == 0) {
    return -1; //value of 0 height when nothing is there
  }
  else {
    right_depth = height_of_tree(bst_node->right); //start at bottom and count upwards
    left_depth = height_of_tree(bst_node->left); //return larger value 
...
int bst_path_sum(int sum, struct bst* bst) {
  return has_path(sum, bst->root);
}

/*
This function will read in the left and right branches and the sum in order 
to find if there is a path to the root from the leaf
*/

int has_path(int sum, struct bst_node* bst_node) {
  if(bst_node == NULL) {
    return 0; //this is saying it's false because there's no path sum that gets you to NULL
  }

  else {
    int path = 0;
    int subtree_sum = sum - bst_node->val; //if it's at a leaf and subtree_sum != 0 then there's not a path

    if(subtree_sum == 0 && bst_node->right == NULL && bst_node->left == NULL) {
      return 1; //if it's a leaf and it's 0 then return true because there is a path sum
    }
    if(bst_node->right) {
      path = path || has_path(subtree_sum, bst_node->right); //can be true or false if true reach base case
    } //or always path to be set to true or false will be true if has_path is true
    if(bst_node->left) {
      path = path || has_path(subtree_sum, bst_node->left); //if any node is true then the whole tree is true which is why it's true or false
    }
    return path;
  }
}
//subtracts node value from the overall sum to get the subtree sum
//if subtree sum is 0 and it's a child node then there is a path
//if right node has a path return true and the whole tree is true
//same with left side  
...
int bst_iterator_has_next(struct bst_iterator* iter) {
  if(stack_isempty(iter->stack)) {
    slide_left(iter->stack, iter->bst->root); //slide all the way down the left side
  } else { //stack is no longer empty
    struct bst_node* node = stack_pop(iter->stack); //pop off the stack
    slide_left(iter->stack, node->right); //check for right child if there is a right child then push then pop
  }
  if(!stack_isempty(iter->stack)) {
    return 1; //this prints the node and calls iter next
  } else {
    return 0;
  }
}
...
/*
This function will slide all the way down the left side check if the left child is ever NULL
if the left child is NULL pop of the parent and then go down the right and check if left is NULL
if left is NULL then pop off the node we are at and eventually make your way back up to the op and go
down the right side because the left side of the root will be NULL now 
*/

/*
this function will go down the left side of the tree
*/
void slide_left(struct stack* stack, struct bst_node* node) {
...
int bst_iterator_next(struct bst_iterator* iter) {
  int* pointer_return_val = (int*)(stack_top(iter->stack));
  int return_val = *pointer_return_val;
  return return_val;
}